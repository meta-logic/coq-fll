...
Section CutCoherence.
  ...
  Theorem CutCoherenceQ (R: QEnc) (FX FX' : uexp -> uexp) (n : nat) :
    ...
    seq (CutRulePOSNEGN n) [] [] (> [ dual (QDefs R Left FX) ;  dual ((QDefs R Right FX') ) ]) .
  Proof with solveF.
    intros.
    destruct R; simpl.
    ++ solveLL.
       decide3 (RCUTPOSNEG (FX x)).
       apply @ctn with (m:=n)...
...
Section OLCutElimination.
  ...
  *)
  ...
  Proof with CutTacPOSNEG.
    ...
    assert(CC: seq (CutRulePOSNEGN n0) [] [] (> [(QDefs T Left FX) ^;  (QDefs T Right FX') ^])).
    {
    ...
    apply WeakTheory with (theory := CutRulePOSNEGN (pred n)); auto using TheoryEmb2...
